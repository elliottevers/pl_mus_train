...
export declare namespace note {
    class Note implements p.parse.Parsable {
        pitch: number;
        beat_start: number;
        ...
        get_best_candidate(list_candidate_note: TreeModel.Node<Note>[]): any;
        choose(): boolean;
    }
    class NoteIterator {
        private notes;
        ...
        next(): {
            value: TreeModel.Node<Note>;
            done: boolean;
        };
        current(): TreeModel.Node<Note>;
    }
}
